...
breadcrumb-title: ''
description: 了解如何使用Substance 3D Painter中的图层堆叠来组织和管理多个纹理绘画图层。
...
使用&#x200B;**图层堆叠**&#x200B;可以操作纹理集的图层。 图层包含绘画和各种效果，可在场景中的3D对象上创建纹理。 您可以隐藏和取消隐藏图层，将它们放入文件夹中并更改其不透明度和混合模式。
...
* 每个图层为&#x200B;**多通道**。
* 绘画工具将在其各自的所有声道&#x200B;**上绘画**，具体取决于材料设置（您当前在图层堆叠中查看的声道没有影响）。
* 每个图层都有&#x200B;**混合模式**&#x200B;和每个通道&#x200B;**不透明度**（您可以通过左上角的下拉菜单在通道之间切换）。

**图层的类型：**

* **绘画图层** ：此类型的图层可以使用画笔和粒子进行绘画
* **填充图层** ：无法在此图层上绘制，您可以将材料加载到其中来填充通道。 （例如，也可以通过操控变换来重复材料。）
* **文件夹** ：此类型的图层仅用于包含其他图层，主要用于整理图层堆叠

在每个图层上，您可以&#x200B;**添加蒙版**，以便仅将内容应用于当前纹理集通道的特定部分。\
您可以手动绘画蒙版（使用画笔进行灰度）或使用滤镜和物质来获得更动态/程序化的结果。

## 视图模式

![](../../assets/switch-viewmode-optim.gif)

图层堆叠的左上角下拉菜单控制图层堆叠的查看模式。 由于图层可以覆盖多个通道，因此不可能同时显示所有这些属性。 因此，可使用视图模式来定义当前显示上下文。 使用此下拉菜单时，可以指定应使用哪些通道在图层缩览图中显示，以及仅控制此通道的混合模式和不透明度。

此下拉菜单中的列表基于[纹理集设置](../texture-set/texture-set-settings.md)中可用的声道列表。

## 操作

![](../../assets/image2020-9-30-12-2-13.png)

图标的右上角列表是可以在图层堆叠中执行的常见操作：
...
| 创建新绘画图层 <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table_row-r3-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/image2020-9-30-11-52-41.png"/></div> | 在当前选定的绘画图层之上创建一个新路径。 |
| 创建新填充图层 <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table_row-r4-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/image2020-9-30-12-0-49.png"/></div> | 在当前所选的[填充图层](../../painting/fill-projections/fill-projections.md)的上方创建新的格式。 |
| 添加新智能材质 <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table_row-r5-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/icon-smartmat.png"/></div> | 在当前选定的图层上方插入新智能材质。单击此按钮将打开一个微型工具架，以浏览当前[资源](../../interface/assets/assets.md)中可用的智能材质列表。 |
