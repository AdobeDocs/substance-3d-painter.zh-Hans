...
breadcrumb-title: ''
description: 了解如何在Substance 3D Painter中配置纹理集设置，以控制纹理分辨率和声道属性。
...
**纹理集设置**&#x200B;控制当前所选纹理集的参数。 在此处可以管理分辨率、通道和相关网格图。
...
| 设置 | 描述 |
| --- | --- |
| **名称** | 纹理集的名称。 已继承3D模型上指定的材料名称。 |
| **描述** | 允许添加有关纹理集信息的文本字段。 此文本显示在[纹理集列表](texture-set-list.md)和[烘焙](../../baking/baking.md)窗口中。 |
| **大小** | 控制纹理集内的声道分辨率（以像素为单位）。 若要使用&#x200B;**非方形**&#x200B;分辨率（例如2048x1024），请禁用两个下拉菜单之间的&#x200B;**锁定按钮**。由于&#x200B;**非破坏性纹理集**，工作流分辨率为&#x200B;**动态**。 这意味着可以在低分辨率下工作，以获得较好的性能，然后使用更高的分辨率来获得更好的质量。 在应用程序内，通道的最大分辨率为4096x4096像素，而导出时的最大分辨率为8192x8192（如果GPU支持）。 更改分辨率可能会触发引擎的长期计算。 |
| **着色器实例** | 定义在[视口](../viewport/viewport.md)中使用哪个[着色器](../shader-settings/shader-settings.md)来呈现给定纹理集。 |
...
这些设置控制有关声道生成方式的各种行为，特别是声道与纹理(网格图)的组合方式。

| 设置 | 描述 |
| --- | --- |
| **正常混合** | 控制“烘焙的法线图”应与“正常”声道组合的方式。 可能的值为：<ul data-preserve-html="true"><li data-preserve-html="true"><strong>替换</strong> ：忽略“烘焙的法线图”并将仅对此纹理集使用“正常”通道。 可用于在烘焙的法线图上绘画。 有关详细信息，请参阅[高级通道绘画](../../painting/advanced-channel-painting/normal-map-painting.md)文档。 如果正常声道不存在，或者正常声道输出为空，则仍将使用烘焙的法线图。</li><li data-preserve-html="true"><strong>合并</strong>（默认） ：使用面向细节的函数合并“正常”法线图和“烘焙的通道”。</li></ul>  **注意：**&#x200B;如果频道列表中缺少该频道，此设置可能被禁用。 如果通道缺失，则使用默认混合值。 |
| **正常方法Height** | 控制使用哪种方法将Height声道转换为法线图。 可能的值为：<ul data-preserve-html="true"><li data-preserve-html="true"><strong>Sharp</strong>：生成定义更明确的法线图，但可能会引入噪声和别名。 适用于重复图案，如织物。</li><li data-preserve-html="true"><strong>平滑(Sobel)</strong>（默认）：使用Sobel滤镜生成更平滑的法线图，但可能会丢失细节。 适用于大多数情况。</li></ul> |
| **Ambient occlusion混合** | 控制“烘焙的ambient occlusion”应如何与“Ambient occlusion”声道组合。 可能的值为：<ul data-preserve-html="true"><li data-preserve-html="true"><strong>替换</strong> ：忽略“烘焙的ambient occlusion”并将仅对此纹理集使用“Ambient occlusion”频道。 可用于在烘焙的ambient occlusion上绘画。 有关详细信息，请参阅[高级通道绘画](../../painting/advanced-channel-painting/ambient-occlusion-painting.md)文档。  </li><li data-preserve-html="true"><strong>乘</strong>（默认） ：使用乘法操作将“Ambient occlusion”ambient occlusion和“烘焙的通道”组合。  </li></ul>  **注意：**&#x200B;如果频道列表中缺少该频道，此设置可能被禁用。 如果通道缺失，则使用默认混合值。 |
| **UV填充** | 控制如何生成UV 岛外部的填充。 可能的值为：  <ul class="steps" data-preserve-html="true"> <li class="step" data-preserve-html="true">    <strong>3D空间相邻像素</strong>（默认）：查看接缝的另一侧以查找相邻像素颜色并将其用于UV边框。 建议在具有连续图案的接缝上绘画时使用此设置。 左侧为常规填充，右侧为3D邻居示例： <br/>       <img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table1_row-r4-column-c1_procedure_proc_par_proc_step_step_par_image_1671759737" src="../../assets/seam-3.png" width="450px"/>   </li> <li class="step" data-preserve-html="true">    <strong>2D空间邻居</strong>：生成内边距之前，将UV 岛内的像素复制到UV 岛外的边框。 当UV 岛的信息非常对立并且不会重叠时，建议使用此设置。 例如，在球体中，每个色带的颜色在UV 岛上是唯一的，左边是2D邻居，右边是3D邻居（注意渗色）：<br/>       <img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table1_row-r4-column-c1_procedure_proc_par_proc_step1_step_par_image" src="../../assets/2d-neigbor-padding.png" width="450px"/>   </li> </ul>  **注意：**&#x200B;此内边距设置是按每个纹理集保存的，并在纹理导出和可视化到视口时予以考虑。由于3D空间邻居的工作方式，它不能与普通通道一起使用，而将使用2D版本。 |

## 网格图

![](../../assets/mesh-maps.png)

这些网格图是特定于网格和纹理集的烘焙纹理，用于借助滤镜、智能材质和智能蒙版提高纹理质量。 有关更多详细信息，请参阅[烘焙](../../baking/baking.md)文档。
