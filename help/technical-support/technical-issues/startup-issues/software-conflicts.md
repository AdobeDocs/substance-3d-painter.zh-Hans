...
本页列出了其他可能崩溃或阻止Substance 3D Painter正确运行的软件的已知问题。
...
| **AMD CrossFire和NVIDIA SLI** | Painter不支持多个GPU配置，这导致崩溃。 我们建议禁用此功能。 |
| <b> Autodesk助手</b> | Autodesk Assistant应用程序可以在启动或打开项目文件时创建冲突并使应用程序崩溃。 更新Autodesk应用程序以解决问题。 |
| <b>台Alienware/Dell计算机</b> | 请参阅此页面以了解更多信息： [打开或保存文件时崩溃](../stability-issues/crash-when-opening-or-saving-a-file.md)。 |
| **APFS by Paragon Software** | 该软件可以在Windows Path环境变量中注册一个位置，以便在启动时崩溃应用程序。 卸载软件可能还不够，可能需要手动删除环境变量。 有问题的位置示例： `C:Program Files (x86)Paragon SoftwareAPFS for Windowsï–›éŒ à €è¸€ì‡ì‡ç¿¹` |
| **Avecto** | 运行Avecto的旧版本可能导致速度减慢和崩溃。 确保将其更新到最新版本。 |
| **华硕GPU调整** | 该软件可能会导致Substance 3D Painter中的着色器编译期间出现问题，甚至会阻止着色器编译启动。 如果遇到此问题，我们建议卸载软件以查看它是否修复了问题。 |
...
| **云备份软件** **（** OneDrive，**GDrive，** **Dropbox，** **文件流等）** | 在保存项目时，云备份软件可以成为大量崩溃的来源。 如果发生这种情况，建议在不再进行更改时处理项目文件，并将项目文件保存到未同步的文件夹，改为将项目文件复制回云驱动器。 |
| **复选框** | 该软件会在打开文件对话框（如打开或保存项目）时制造冲突并崩溃应用程序。 您可以在复选框首选项中禁用设置<b>启用桌面模型的缩览图预览</b>以避免此问题。 |
| **Duet显示** | <b>Duet Display</b>已知会产生GPU驱动程序问题，可能会影响Substance 3D Painter的行为。 建议卸载它。 |
| **Google Chrome** | 与Substance 3D Painter一起运行时，Google Chrome可能会导致一些崩溃。 要提高Substance 3D Painter的稳定性，建议您更新Google Chrome和GPU驱动程序。 如果仍然出现崩溃，请在Google Chrome中禁用硬件加速（这将阻止Chrome使用GPU）。 |
| **Nahimic音频软件** | <b>Nahimic</b>可以冻结或Painter。 停止它会有帮助，更新它也会避免问题。 Nahimic还会运行后台服务，这些服务可能会干扰应用程序，可能需要停止或禁用。 |
| **Openshot视频软件** | <b>Openshot视频软件</b>可能会使用工具架预览与Substance 3D Painter发生冲突。 更新Openshot应该可以修复此问题。 |
| **Pyinstaller** | 此应用程序会产生不正确的环境设置，导致启动时出错。 有关详细信息，请参阅[应用程序因Qt](application-failed-to-start-because-of-qt.md)而无法启动。 |
| **Rptr / Plays.tv** | 默认情况下，<b>Rptr</b>(或<b>[Plays.tv](http://plays.tv/) </b>)与某些GPU驱动程序一起安装。 此软件可能会造成不稳定并崩溃应用程序。 建议卸载该应用程序。 |
