...
&#x200B;* [倾斜烘焙]倾斜绘画工具
&#x200B;* [倾斜烘焙]绘制倾斜映射时添加倾斜预览着色器和倾斜方向矢量视觉效果
&#x200B;* [倾斜烘焙]添加边缘保护选项
&#x200B;* [倾斜烘焙]自动重新生成
&#x200B;* [倾斜烘焙]返工网格图列表UI
&#x200B;* [倾斜烘焙]拆分网格图/通用烘焙设置+将通用设置移出网格图列表基色或仅蒙版
&#x200B;* [倾斜烘焙]更改视口工具栏按钮
&#x200B;* [倾斜烘焙]在顶部工具栏中显示画笔的对称切换
&#x200B;* [倾斜烘焙]网格图列表同步菜单中的重命名选项
&#x200B;* [倾斜烘焙]“更新同步”和“已检查状态”对话框
&#x200B;* [倾斜烘焙]创建灰度拾色器变体
&#x200B;* [倾斜烘焙]更新烘焙模式图标
&#x200B;* [自动展开]“集成硬表面”选项
&#x200B;* [OpenPBR]添加对OpenPBR1.1的支持
&#x200B;* [OpenPBR]将OpenPBR设置为默认工作流程和着色器
&#x200B;* [OpenPBR]通过USD导入OpenPBR材料和纹理
&#x200B;* [OpenPBR]通过USD导出OpenPBR材料和纹理
...
&#x200B;* [崩溃]在不同纹理集上使用拾色器可能会导致在退出应用程序时出现崩溃
&#x200B;* [性能]在具有多个图层的项目中绘画时出现性能问题
&#x200B;* [性能]使用图形平板电脑笔绘画时出现滞后
&#x200B;* [UI]相机设置在渲染模式下保持禁用状态(Iray)
&#x200B;* [功能区]在某些情况下，路径在拐角后可能会意外地自动重叠
&#x200B;* [功能区]UV 平铺出现性能问题
&#x200B;* [Substance]&#x200B;[UI]图像输入在折叠时消失
&#x200B;* [Substance]&#x200B;[UI]即使嵌套组已隐藏，它们也可以保持可见
&#x200B;* [烘焙]&#x200B;[UI]无法设置超过0.01的弯曲取样半径
&#x200B;* [烘焙]&#x200B;[UI]无法设置最大遮挡板距离超过1
&#x200B;* [烘焙] AO设置“自遮蔽”被忽略，带有多个纹理集且“低”为“高”烘焙
&#x200B;* [烘焙]在“低”和“高”模式下，Id 图不会烘焙FBX中的顶点颜色
...
&#x200B;* [性能]更新了Baker，具有更快的GPU渲染和对AMD射线追踪的支持
&#x200B;* [UI]默认情况下，将工具属性重新组织为组并折叠一些属性
&#x200B;* [引擎]将Substance 引擎更新至版本9.2.5
&#x200B;* [Substance]在“工具”和“填充”中覆盖Substance资源的公开分辨率
&#x200B;* [导出]更新网格图导出预设以导出灰度纹理
&#x200B;* Python
&#x200B;* [烘焙]&#x200B;[Python]指示在Baker更新后更改日志中断更改
&#x200B;* [Python]公开Python中的填充对称设置
&#x200B;* 内容和新内容
&#x200B;* [Content]为功能区工具添加75种新工具预设
&#x200B;* [内容]更新渐变生成器资源以便与功能区兼容

<b>已修复</b>：

&#x200B;* [崩溃]在启用捕捉路径时加载其他项目可以崩溃
&#x200B;* [崩溃]可使用剪贴板中来自其他会话的信息在“路径”面板中单击鼠标右键即可崩溃
&#x200B;* [UI]创建路径时，界面在工具属性中向上滚动
&#x200B;* [UI]路径视口可视化效果处于隐藏状态时，鼠标光标消失
...
&#x200B;* [Path]从侧视图改进正切版本
&#x200B;* [路径]允许将焦点放在3D路径上
&#x200B;* [Path]在关闭和再次打开UI时，顶点应保留选择状态
&#x200B;* [Path]允许使用退格键删除路径
&#x200B;* [路径]如果用户展开路径列表，请将其保持为打开状态
&#x200B;* [Path]&#x200B;[图层堆叠]复制/粘贴时正确重命名重复项
&#x200B;* [路径] UI和工具提示改进
&#x200B;* Performance
&#x200B;* [性能]在使用高曲面细分级别时提高视口性能
&#x200B;* [性能]仅在新填充图层/效果上启用第一个通道
&#x200B;* [性能]并行画笔描边计算
&#x200B;* Baking
&#x200B;* [烘焙]为高多边形笼子的烘焙添加新的全自动网格生成选项（实验性）
&#x200B;* 内容
...
&#x200B;* [图层堆叠]在蒙版中使用不兼容滤镜时出现错误信息
&#x200B;* [图层堆叠]删除后，选区会切换回顶层图层
&#x200B;* [导出]生成的正常纹理始终处于3D空间相邻像素间距模式
&#x200B;* [导出]未使用2D 视图导出预设生成纹理alpha
&#x200B;* [导出] SBSAR导出在转换的地图中使用不正确
&#x200B;* [着色器]着色器 API更改日志未包含最新的ASM更改
...
&#x200B;* [互操作]允许将带纹理的网格发送到After Effects (Ae 24.1)

<b>已修复：</b>

&#x200B;* [Fill]UV 集投影时，读取的UV 集不超过2个
&#x200B;* [崩溃]使用16K环境图
&#x200B;* [崩溃] Exr用作图像输入
&#x200B;* [崩溃]跨项目复制和粘贴路径
...
&#x200B;* [USD]&#x200B;[Export]添加导出USD文件的选项
  ...
  “导出纹理”窗口中添加了新的“导出USD”复选框。 选中后，即可使用任意模板导出美元文件和纹理图。
...
&#x200B;* [颜色选择]将混合模式添加到颜色选择效果
&#x200B;* [图层堆叠]在滤镜上添加混合模式和不透明度
&#x200B;* [图层堆叠]允许拼贴值大于128的填充图层/效果
&#x200B;* [图层堆叠]用于填充图层/效果中圆柱投影的气缸盖

  填充图层属性中的圆柱投影现在可以选择移除圆柱盖。

&#x200B;* [日志]在尝试创建UV 平铺项目时，如果网格部件位于负空间中，则显示错误消息

  当由于负空间而未能创建UV 平铺项目时，添加了更清晰的错误消息。UV部分已被删除。
...
&#x200B;* [内容]添加新的Roblox模板并导出预设
  ...
  添加了一个新的Roblox“材料变体”和“表面外观”项目模板以及导出预设，以便更轻松地将PBR纹理导出到Roblox。 可通过“文件”>“新建项目”窗口访问该模板。
...
&#x200B;* [Python]导入函数返回的资源对象没有类型
  ...
  在Python中导入Substance包时，函数返回的是包而不是其图形。 资源模块现在提供用于检索Substance包图形的功能和参数。
...
&#x200B;* [自动展开]新增选项“优化有机网格”以选择分段算法
&#x200B;* [物理尺寸]在新项目和项目配置中公开设备选项
&#x200B;* [色彩管理]使用ACE时默认使用显示器显示
&#x200B;* [色彩管理]&#x200B;[Python]创建项目时考虑到ACE env-var预设文件
...
&#x200B;* [Scripting]&#x200B;[Python]向每个纹理集的网格名称查询列表添加函数
&#x200B;* [内容]添加新的混合器模板并导出预设

**已修复：**

&#x200B;* [MacOS]在某些情况下，启动Iray时会崩溃
&#x200B;* [缩略图]无法正确加载工具架缩略图
&#x200B;* 多个UV通道将被忽略
&#x200B;* [自动展开]拆分长岛时不必要的计算
...
&#x200B;* [内容]使用货架中的随机种子监管更新内容
&#x200B;* [Content]工具架中带有公开随机植入参数的新图标

**已修复：**

&#x200B;* [图层堆叠]图层堆叠不断拖动
&#x200B;* [Mac] “在Finder中显示”可能会导致冻结
&#x200B;* [脚本]移动着色器文件时，通过自定义UI保存的设置会丢失
&#x200B;* [脚本] API版本号不正确，不是最新的
&#x200B;* [效果]直方图内容无法正确显示
&#x200B;* [效果]在某些情况下，直方图效果不会更新
&#x200B;* [工具架]材料“塑料织物金字塔”上的针脚未正确对齐
