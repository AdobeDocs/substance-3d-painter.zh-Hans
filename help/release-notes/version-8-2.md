...
添加了一些快捷键和操作，可快速轻松地在图层堆叠中的多个通道之间复制和应用混合模式和不透明度。
...
* **颜色选择上的混合模式和不透明度**\
  已修改颜色选择效果以支持混合模式和不透明度控制。 以前，此效果是输出Alpha结果，为了使混合模式按预期工作，已添加新设置以指定正在输出的背景色。 它设置为黑色而非透明（这是旧版行为）。
...
Substance存档(SBSAR)文件格式现在可用于导出纹理。 SBSAR是一种可通过Substance集成在许多应用中打开的容器，它可以使自定义纹理更快捷、更易于“即插即用”。

* **导出Substance存档(SBSAR)**\
  现在可以从&#x200B;**导出纹理**&#x200B;窗口的文件格式列表中指定SBSAR文件格式。 这将导出包含所有指定纹理的单个Sbsar 文件。 输出节点的命名及其使用情况根据选定的导出预设及其通道类型来定义。

  ![](../assets/sbsar-export.png)

* **具有PSD和Sbsar 文件格式的混合导出预设**\
  导出预设现在可将输出映射指定为PSD或SBSAR，以及所有其他图像格式。 PSD和SBSAR格式被视为“容器”，这意味着可以在其中存储多个纹理。 当导出预设同时指定容器格式和独立图像格式时，模板中针对某一Sbsar 文件的每个输出将组合在一起，而其他输出将导出为单个文件。
...
[显示设置](../interface/display-settings/environment-settings.md)中的新设置允许将环境图与相机对齐，从而可调整光照角度和3D模型下方的照亮部分。
...
* **删除并重新加载磁盘上的资源**\
  现在可以删除、重新加载或重命名用户库中的资源（包中的资源部分，如图形或ABR画笔除外）。
...
* **用于快速重新导入3D模型的新操作**\
  已添加新的键盘快捷键（默认情况下为&#x200B;**CTRL+SHIFT+R**），允许快速重新导入当前项目的3D模型。 这样可更轻松、更快速地迭代资源。 如果找不到源文件，将在日志中引发一条错误消息。 还向&#x200B;**编辑**&#x200B;菜单添加了操作。
...
* **将图形参数重置为默认值**\
  在所有使用图形的地方（作为Alpha、材料、滤镜等） 现在可以将其参数重置为默认值。
...
* **查看视口中的单个RGBA组件**\
  查看视口中的通道时，**显示设置>通道显示**&#x200B;下有一个名为&#x200B;**颜色通道**&#x200B;的新设置，允许单独查看RGBA组件。 这对于分析纹理或隔离用户通道中的特定组件非常有用。
...
* **拼贴128度以上的填充图层和效果**\
  填充图层和效果的拼贴参数已修改为具有软范围。 这样即可键入任何所需的拼贴值。 滑块的默认范围还从[-128,128]缩小到[-32,32]，以便更轻松地拖动。

  ![](../assets/fill-tiling.gif)

* **新的16f和32f 纹理导出设置**\
  以前，纹理导出在接口中被强制为32f位，但在实际文件中，它将产生16f位数据（半浮点）。 此问题现已修复，在16f和32f位之间有明确的可能性可供选择。 旧项目和使用EXR作为文件格式导出预设的文件格式将默认为16f位，以遵循旧行为（主要是为了避免生成比以前更大的文件）。
...
* **substance\_painter.event**\
  已添加新的&#x200B;**TextureStateEvent**，以帮助跟踪纹理集图层栈栈中的修改以及其他参数更改。 此事件触发绘画描边或添加/删除声道。
...
* [Shader]允许在素材图层着色器中将用户通道设置为RGBA
  ...
  当在用于纹理集图层的着色器内设置材料声道配置时，现在可以指定声道的格式以偏离默认值。 这尤其允许您请求彩色用户通道，而不是仅请求灰度。
* [导出]允许将纹理导出为SBSAR
...
* [Python]允许在纹理集设置中获取和设置网格映射资源
  ...
  在“substance\_painter.project”模块中添加了新的函数，用于获取和设置网格图资源。 这些函数可用于更新纹理集设置所引用的网格图。
* [插件]删除获取其他JS插件的选项
