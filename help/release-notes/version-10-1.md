---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/release-notes/version-10-1.html"
breadcrumb-title: ''
...
* <b>新的刺绣贴花材料</b>\
  在“资源”窗口的“材料”部分中，您可以找到新的“刺绣贴花”材料。 将它拖放到您网格上的任意位置，插入任意资源（如纹理甚至字体），您将能够轻松地创建新的结构详细信息。
...
* <b>新FXAA筛选器</b>\
  此新滤镜可以快速减少锯齿，特别是在硬边缘（例如，在色阶之后可能会出现的硬边缘）或使用颜色选择效果制作的蒙版上。

  ![](../assets/v101_filter_fxaa.gif)
* <b>新的高反差滤镜</b>\
  使用此通用滤镜，您可以生成灰度纹理以用于更高级的效果（如柔化、模糊或锐化细节）。
...
* <b>在USD中导出带Adobe Standard Material的纹理\
  </b>现在，当您将纹理从Painter导出到USD文件中时，将获得这些属性的Adobe Standard Material属性。 这使这些USD文件可以准备就绪用于支持这些属性的应用程序。
* <b>从USD文件导入纹理</b>\
  现在，导入USD文件也会将其纹理导入到它创建的项目中，从而更轻松地在应用程序之间来回切换。 如果USD文件使用Adobe Standard Material，这也将配置着色器设置，使视口中的结果与其他源应用程序匹配。
* <b>Gltf更改\
  </b>在USD更新后，需要对GLTF格式的行为进行一些更改以确保奇偶校验。 在导入gltf文件时，Painter现在会假定法线图将采用OpenGL格式。\
...
* <b>改进了镶嵌性能\
  </b>现在，当在着色器设置中启用镶嵌时，我们使用自动优化。 小于屏幕上的像素的三角形将不再进行镶嵌，从而减少绘制的三角形，进而加快渲染时间。\
  此更改不会产生视觉差异，也不会影响网格导出过程。
* <b>简化缩览图现在是默认值</b>\
  在6.2版中，我们为UV 平铺项目引入了简化的缩览图以提高性能，但常规项目仍然可以使用旧的计算图层缩览图方式。 此行为是通过应用程序设置控制的。\
