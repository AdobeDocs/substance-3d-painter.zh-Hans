---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/release-notes/version-11-0.html"
breadcrumb-title: ''
...
<b>Substance 3D Painter 11.0</b>添加了新的自动资源更新工作流程、填充路径工具以及路径的一般改进、自动烘焙笼子和用于创建风格化纹理的几个新过滤器。
...
* <b>在Python API中可用于自动进行项目维护</b>\
  自动更新工作流程也已公开到Python。 新增了新功能，帮助列出并替换过时的资源。\
...
填充路径工具是一种新型的路径工具，允许在填充统一颜色的3D模型表面创建形状。 它使复杂图案的创建成为可能。

* <b>用于创建具有填充颜色的路径的新工具</b>\
  “路径”菜单中提供了一个名为<b>填充路径</b>的新工具。 闭合时，此工具可填充路径的内部区域。 填充是通过对每个纹理集通道的统一颜色完成的。

  ![](../assets/filled_path_menu.png)
* <b>自动适应表面</b>\
  “填充路径”工具可以适应任何类型的曲面，它不限于平面区域。 它可以跨越间隙和对象边界。

  ![](../assets/filled_path_across_gaps.png)
* <b>与镜像和径向对称兼容</b>\
  这一新工具还支持对称属性，从而提供了创建复杂形状的可能性。
...
* <b>直线和角度捕捉</b>\
  <b>Shift </b>键盘功能键现在可用于自动在点之间创建直线。 保持<b>Ctrl </b>也可用于应用角度捕捉，这有助于构建几何形状。\
  角度捕捉设置可以通过上下文工具栏中的“路径设置”菜单修改。

  ![](../assets/path_angle.gif)
* <b>将路径点捕捉到网格多边形</b>\
  为了更轻松地放置点，可以启用新的捕捉（磁铁图标）。 此选项允许将点放在3D模型顶点上并跟随曲面或边。\
  捕捉可以通过三种方式完成：
...
* <b>单击上一个顶点时自动关闭</b>\
  为使<b>填充路径工具</b>更易于使用，在选中最后一个工具时单击第一个顶点，现在将自动关闭该路径。 若要选择点而不是闭合路径，可以使用<b>CTRL </b>键。 （此行为在早期版本中反转。）

  ![](../assets/path_close_optim.gif)
* <b>将路径顶点位置从内容复制到蒙版</b>\
  现在可以执行<b>复制</b>材质模式中的路径，然后使用蒙版中的路径上的<b>粘贴所有顶点</b>。 这样就可以在材料和蒙版之间同步不同的路径。

  ![](../assets/paste_vertices.png)
* <b>改进了显示/隐藏UI显示行为</b>\
  现在，按下操纵器键盘快捷键（<b>W</b>、<b>S</b>或<b>D</b>）即可实时切换它们。 还可通过上下文工具栏专用按钮启用/禁用它们。 此更改使您可以快速显示或隐藏这些视觉效果，而无需隐藏视区中的其他视觉效果（如路径曲线和点）。

  ![](../assets/manipulator_toggle.png)
* <b>现在可在路径顶点上访问旋转和缩放</b>\
  在此版本中，现在可以在选择多个顶点时使用<b>旋转</b>和<b>缩放</b>工具。 这样就可以一起调整和对齐顶点。

  ![](../assets/path_scale_vertices.gif)
* <b>在“属性”窗口中显示路径信息</b>\
  当选择路径工具时，“属性”窗口现在会有一个新部分。 这一新部分将特定于路径的信息和操作重新分组，如路径长度、深度以及在类型之间切换的操作。

  ![](../assets/path_properties_info.png)
* <b>从某个角度查看时改进了Tangent版本</b>\
  编辑自定切线可能很困难，具体取决于视图角度。 现在，这一点已有所改变，以便正切将受限于他们自己的计划。

  ![](../assets/path_tangent_angle.gif)
* <b>在图层之间保持路径列表打开</b>\
  在不同的绘画图层和效果之间切换时，如果视口中的“路径”面板已关闭，则在其他图层上它也会保持关闭状态。 面板现在将保持打开状态，以便更方便地来回切换。

  ![](../assets/path_panel.png)
* <b>聚焦当前选定的路径</b>\
  编辑路径时，按下<b>F</b>键盘快捷键现在将专注于路径，而不是整个3D模型。
* <b>删除具有退格空间的路径</b>\
  现在可以通过按<b>退格键</b>键盘快捷键快速删除路径。

### 新的Substance滤镜和纹理生成器

![](../assets/banner_filters.jpg)

新版本引入了几个新的滤镜以及一些程序化的图案。
...
* <b>量化</b>\
  量化滤波器可用于减少图像中的颜色数量并创建具有硬限制的平坦区域。 它还可以用于设置纹理的风格。

  ![](../assets/filter_quantize.jpg)
* <b>各向异性科威特语</b>\
  此滤镜应用[Kuwahara滤镜](https://en.wikipedia.org/wiki/Kuwahara_filter "https://en.wikipedia.org/wiki/Kuwahara_filter")，此滤镜还可用于减少噪声和设置纹理风格。

  ![](../assets/filter_kuwahara.jpg)
* <b>方向距离</b>\
  这是一个简单的滤镜，用于在2D空间中以指定方向拉伸像素。 它可用于涂抹画笔描边或轻松创建漏口。
...
* <b>Triangle Grid</b>\
  通过三角形连接构建的噪声，其中包含随机性和Smoothness控件。
* <b>平铺随机</b>\
  一种专为建筑拼贴图案定制的纹理生成器。
* <b>Voronoi和Voronoi分形噪声</b>\
  这些新的2D版本已可用作3D噪声，可用于在2D或UV空间中进行处理和拼贴。
* <b>已将噪声从Designer </b>更新到最新版本\
  Painter中的大多数噪声已使用Substance 3D Designer的最新版本进行了更新。 噪声参数不再隐藏在组中，以便更快地编辑。
...
将高多边形烘焙为低多边形网格时，现在可以在指定笼子模式时选择新的<b>自动</b>选项。 该方法试图计算一个自动的网格，它最适合高多边形网格以避免伪影。

* <b>常用烘焙参数</b>中的新设置\
  在常用烘焙参数中，笼子参数已被替换为三个选项之间的选择：\
  <b>基于距离</b>：默认的正/后距离设置。\
  <b>自动（实验性）</b>：新的自动笼子。\
  <b>自定义文件</b>：以前将自定义网格文件加载为笼子的方式。
...
* <b>现在使用Metal图形API，而不是Mac </b>上的OpenGL\
  从此版本开始，Painter现在在Mac上使用<b>Metal </b>图形API来渲染其视口和计算纹理。 该开关极大地提高了应用的性能和稳定性。 将来还会更容易集成新功能，因为OpenGL在MacOS上已被弃用。
...
* <b>仅在新填充图层/效果上启用base color频道</b>\
  默认情况下，创建新填充图层或效果时，将仅启用Base color声道。 （当拖放资源以使其本身创建填充图层/效果时，此更改不适用。）\
  根据社区反馈，我们进行了此更改，以避免触发随后被禁用的通道的计算，从而改善性能。 当使用高分辨率或UV磁贴时，这有助于响应度。\
  请注意，在保持<b>ALT </b>键盘快捷键不变的同时，单击“基色”按钮可快速重新启用所有通道。

  ![](../assets/base_color_single_channel.png)
* <b>重命名用于导出纹理的UV 平铺</b>\
  在“纹理集列表”窗口中，不能在UV 平铺上添加自定义名称。 与说明相反，可以通过专用标记<b>$uvTileName</b>在导出预设中检索自定义名称。\
...
* [Nvidia]&#x200B;[烘焙]Ambient occlusionBaker结果包含伪像
* [崩溃]按住Alt键单击可在禁用的纹理集线索上切换可见性以导致崩溃
* [烘焙]以低多边形作为高多边形参数考虑笼子
* [烘焙] ID map Baker的材质颜色无法与USD文件格式配合使用
* [性能]网格和大量重叠对象视口渲染缓慢
* [Qt]自定义构建的拾色器没有色彩管理设置
* [视口]启用“消除锯齿”后3D操纵器会闪烁
* 蒙版块画笔状态中橡皮擦的灰度槽
* [Log]导入网格时未报告很长的错误消息
* [Content] Topstitches工具预设内的预设名称列表中出现拼写错误
