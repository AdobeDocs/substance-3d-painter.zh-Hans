---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/release-notes/version-10-0.html"
breadcrumb-title: ''
...
<b>Substance 3D Painter 10.0</b>提供了对Illustrator (.ai)文件的支持、集成了Substance 3D Assets、通过文本资源导入字体、在Python API中添加了图层堆叠功能以及一些生活质量改进。
...
此新版本引入了<b>文本资源</b>，它是一种加载字体文件以在不同上下文（画笔、填充投影、Substance图像输入等）中写入文本的方法 来美化你的纹理。
...
* <b>像拖放任何其他资源一样拖放字体</b>\
  字体可以像任何其他类型的资源一样用作文本资源。 拖放它们以自动创建填充投影。 它们还可用于画笔中，或作为Substance过滤器的输入。

  ![](../assets/v10_text_drag_drop.gif)
* <b>文本资源参数</b>\
  创建文本资源时，您可以微调一些参数来调整文本的外观：垂直和水平对齐方式、自动或手动调整大小、行和字符间距、颜色等。
...
* <b>Illustrator (.Ai)文件支持</b>\
  在此新版本中，现在可以在Painter中导入和渲染.ai文件，以便用作画笔、填充投影中的资源或Substance图像输入。
* <b>.svg和.ai文件共享公共设置</b>\
...
此版本会在Python API中添加新的图层堆叠模块。 此API允许您控制项目的图层堆叠，这为创建高级图层堆叠增效工具和自定义工具打开了大门。

* <b>新图层堆叠API</b>\
  新的<b>layerstack</b>模块允许以多种方式控制项目的图层堆叠。 您可以：
  ...
  * 获取和设置Substance参数。
* <b>作用域修改和引擎暂停</b>\
  操作图层堆叠可能会导致计算过长，这就是为什么我们还公开了从API暂停和取消暂停引擎的可能性（例如在用户界面中）。 我们还使修改组合在一起成为可能，这既是为了提高性能，也是为了撤消一次多次操作。
* <b>基本色彩管理</b>\
  通过对图层堆叠的阐述，我们需要在API中引入色彩管理的概念。 已添加新的<b>色彩管理</b>模块，用于创建、调整颜色和选择位图的色彩空间。 （此部分API尚未完成，将在未来版本中扩展。）
* <b>查询导出预设信息</b>\
  导出预设现在公开我们的API中，允许查询预设列表（预定义与自定义）。 也可以用与我们现有的导出纹理API类似的格式检索这些文档的内容。
* <b>前面有新的可能性！\
...
>[!NOTE]
>
> 在我们的[联机文档](https://adobedocs.github.io/painter-python-api/)中也可以找到图层堆叠增效工具的示例。

### 改进了法线图绘画

![](../assets/v10_banner_flow-1.jpg)

在此版本中，我们改进了法线图绘画工作流程。 我们特别改变了积累和混合正常画笔图章的方式。 进行此更改是为了解决与绘画流程图相关的问题。
...
* <b>零Alpha上的透明度</b>\
  使用Alpha值为0的纹理制作的画笔图章现在将绘制为透明。 下面的示例显示了一个画笔图章（左）与一个投影（右）。
...
* 带有CTRL的<b>精度模式</b>\
  现在，在操纵器上拖动时按住Ctrl键将进入新的精确模式，该模式允许进行更精细的操作。 此更改适用于平移、旋转和缩放操纵器。\
  以下是一个在拖动时按住CTRL键前后的示例：

  ![](../assets/v10_ctrl_precise_mode.gif)
* <b>新的缩放行为</b>\
  缩放强度现在基于当前缩放值本身，不再基于场景大小。 这使得相对更改更易于执行，尤其是在较小值的情况下。 与精确模式相结合，缩放效果更令人愉快。\
  另一个更改是缩小值，直到0不再变为负值。 这避免了想要缩小投影规模，然后意外将其翻转的问题。

  ![](../assets/v10_translate_new.gif)
* <b>改进了表面操纵器旋转</b>\
  现在，在曲面周围拖动时，曲面贴花操纵器更加稳定。 在来回进行翻译时，它不会增加旋转角度。\
...
* <b>拖放时对齐相机</b>\
  将资源拖放到视口中可直接在网格表面创建变形投影。 此投影之前旋转不正确，现在已与相机对齐。
...
发行日期：<b>2024/05/16</b>\
摘要：<b>主要版本，使用Python API的图层堆叠，读取本机Illustrator文件，集成3D资源和新文本资源</b>
