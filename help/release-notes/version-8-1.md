---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/release-notes/version-8-1.html"
breadcrumb-title: ''
...
**Substance 3D Painter 8.1**&#x200B;将Adobe 颜色引擎(ACE)与对ICC配置文件、新Baker、新3D噪声和20个污渍映射的支持以及改进的滴管功能集成在一起。
...
材料内的大小现在可用于驱动其在填充图层投影内的缩放和拼贴。 根据曲面上的实际材料大小进行正确匹配时无需猜测，是一种非常有用的工具。

* **新填充图层参数**\
  如果填充图层（或效果）定义了物理尺寸，则有一些新参数可用于控制材料的拼贴/重复。 这些新参数仅适用于3D投影。

  ![](../assets/physical-size-settings-fill.png){width="400px"}

* **新网格**\
  为了使物理尺寸更易于理解和可视化，现在可以通过[显示设置](../interface/display-settings/display-settings.md)窗口在3D视口中激活网格。\
  启用后，网格将根据缩放级别自动细分。 网格单位显示在视口的左下角。
...
它们已添加到Baker列表中，但默认情况下处于禁用状态：

![](../assets/baker-list.png)

新Baker包括：

* **Baker** Bent normalsBaker允许烘焙遮蔽方向（作为矢量，类似于法线图）。 通过在[着色器设置](../interface/shader-settings/shader-settings.md)窗口中启用&#x200B;**弯曲正常**&#x200B;设置，可以使用此纹理来改善视口中的着色。 bent normals大大提高了实时视口着色的精度。\
  对于&#x200B;**漫射着色**，它可以提供更准确的遮蔽，甚至看起来像近似的全局照明（下面第一个示例）。\
  对于&#x200B;**Specular反射**，它允许模拟自阴影并减少光泄露，使物体感觉更接地，特别是对于金属表面（下例第二个）。
...
* **Baker**\
  该Baker允许作为灰度纹理烘焙低和高多角网格之间的差值，然后该灰度颜色可用于在镶嵌网格上产生位移。 例如，针对平面烘焙扫描信息时。

  ![](../assets/disp-demo.jpg){width="400px"}

* **不透明度Baker**\
  不透明度Baker生成的黑白映射图显示来自高多边形网格的洞。 例如，它可以用于烘焙织物表面内的栅栏甚至孔。
...
* **包含超过100种预设的新的和改进的3D噪声**\
  对现有三维噪声进行了改造，增加了三个新的三维噪声。 现在，每个预设集都包含预定义设置，共有7个噪声共105个预设。 可以使用这些预设作为处理其参数并获得特定外观的起点。 与3D噪声始终一样，它们是非常无缝的，可以非常方便地重复而不会出现明显的图案。

  要查找3D噪声，只需转到“资源”面板的“过程”部分：

  ![](../assets/3s-noise-shelf.jpg){width="400px"}

  这些噪声提供了多种可能性，例如&#x200B;**3D Voronoi Fractal**&#x200B;中提供的预设：
