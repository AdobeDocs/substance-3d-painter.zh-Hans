...
现在可以&#x200B;**导出2D 视图**&#x200B;将&#x200B;**呈现为纹理**。 此功能已被许多人请求，我们最终使其可用！ 导出过程将采用&#x200B;**2D 视图**&#x200B;的当前状态，以使用常规导出设置（填充、文件格式、位深度）渲染纹理。 这意味着如果视图模式设置为&#x200B;**独奏**&#x200B;而不是&#x200B;**材料**&#x200B;模式，则2D 视图将按原样导出。
...
您现在可以复制视口的光照（如2D 视图中所示），并将其烘焙到Base color通道中。 新滤镜提供了进一步的控制，如&#x200B;**旋转****环境**&#x200B;映射&#x200B;**垂直**&#x200B;和更改&#x200B;**曝光**。
...
在此新版本中，我们引入了一个名为“**pbr-metal-rough-各向异性-angle**”的新着色器。 此着色器支持两个名为“**Anisotropy angle**”和“**Anisotropy level**”的通道，可用于创建各向异性Specular反射。 此着色器还将按原样转换为Iray，无需任何转换。

通过单击“着色器”按钮并打开mini-shelf ，您可以通过[着色器窗口](../../interface/shader-settings/shader-settings.md)访问此新着色器：

![](../../assets/shader-anisotropy.png)

已更新默认示例项目“**Preview Sphere**”，以利用该新着色器并展示如何设置各个通道。
...
新的&#x200B;**TAA**&#x200B;还将很好地滤镜&#x200B;**Specular反射**&#x200B;和&#x200B;**次表面散射**&#x200B;样本中可见的蓝色噪声图案。
...
这一新的系统改变了Substance Painter的一些基础知识和应用方式。 Substance Painter现在使用SVT为视口保持特定的内存空间，从而允许&#x200B;**流入和流出纹理**。 主要好处是能够更轻松地加载更大的项目，并减少GPU的压力，以&#x200B;**提高性能**。 这意味着，如果问题变得太大，将卸载磁盘上的某些纹理，并在必要时稍后将它们找回)。 这是应用程序关闭时删除的&#x200B;**易失性缓存**。
...
在控制三操纵器时，除了控制旋转的3个原始轴外，还增加了新的旋转球体。 例如，球体使得在投影噪声图案时快速尝试不同角度更加容易。

### 导出8位抖动纹理

![](../../assets/dither-1.jpg)

现在，当以8位模式将普通和纹理导出为文件Substance Painter时，将自动应用&#x200B;**仿色**&#x200B;以减少&#x200B;**条带** **问题**。

>[!NOTE]
>
> 如果导出预设使用法线图，但Alpha中的其他内容（如RGB=正常，A =粗糙度），则只有正常内容会被抖动。

### 图层堆叠行为改进

![](../../assets/color-layers.gif)

对图层堆叠和图层管理进行了一些工作流程改进：

* 通过&#x200B;**右键单击**&#x200B;菜单为图层堆叠内的&#x200B;**图层**&#x200B;和&#x200B;**文件夹**&#x200B;分配&#x200B;**颜色**&#x200B;以整理图层。\
  Substance Painter图层颜色的行为与其他软件包中的行为略有不同，但是：
  * 文件夹中的图层将继承文件夹的颜色（但将显示为灰色）。
  * 如果移动图层时没有在含有颜色的文件夹中分配颜色，则将继承文件夹颜色。
  * 如果图层具有专用颜色，它不会被文件夹覆盖。利用这种原始行为，无需手动指定过多颜色，即可更轻松地为图层堆叠着色和整理。
...
* 使用&#x200B;**箭头**&#x200B;键盘&#x200B;**快捷键**&#x200B;快速&#x200B;**在混合模式**&#x200B;之间切换。\
  在&#x200B;**关闭**&#x200B;混合弹出菜单之后，**焦点**&#x200B;将&#x200B;**保留**&#x200B;在图层上，并且可以使用相同的上一个快捷键继续更改该图层。

### 过滤器和生成器的新Substance输入

![](../../assets/uv-border-generator.gif)

为定制过滤器和发电机公开了新的Substance输入。 这些新的纹理输入借助与网格相关的新信息，可以创建更高级的效果。
...
例如，我们现在提供了一个名为“**UV边框距离**”的新&#x200B;**蒙版生成器**，它从当前纹理集的UV 岛边框创建黑白色蒙版。

![](../../assets/uv-border.png)

>[!NOTE]
>
> 这些输入直接从基于项目网格的Substance Painter引擎中提供，并且不使用[Baker](../../baking/baking.md)。
...
* 新&#x200B;**材料** ：
  ...
  * 人类女性30岁的脸部06（可通过工具架中的皮肤预设快速找到）\
    此新皮肤材料由&#x200B;**Texturing.XYZ**&#x200B;提供，为绘画逼真的皮肤提供出色的表面细节。\
    ![](../../assets/skin-face.png)

我们还更新了一些现有内容以对其进行优化：

* 更新程序筛选器“**烘焙光照环境**”：请参阅上文。
* 更新的滤镜“**MatFx Shutline**” ：现在允许隐藏材料效果并仅保留Height/正常结果。
* 更新了&#x200B;**示例项目** ：预览球面现在可与对称一起使用，并为自定义渲染使用新的相机角度。 其默认着色器现在为“Anisotropy angle”。
...
* 摘要：视口升级、正确的2D 视图导出、新的UI助手、增强的对称工具、新的内容和性能的大幅提升
* [消除锯齿][视口]用于3D视口的新随机采样抗锯齿筛选（通过显示设置）
* [导出]将2D视口的内容导出为单个纹理
* [导出][仿色]导出时公开仿色
* [图层栈栈]图层和文件夹上的颜色
* [图层栈栈]快速激活和停用多个图层和效果
