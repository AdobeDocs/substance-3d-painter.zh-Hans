...
**Substance Painter2019.3**&#x200B;为网格引入了Photoshop笔刷预设支持和自动UV展开，并提供了各种生活质量改进，例如更好地处理图形平板电脑。
...
* [Kyle的笔刷预设](https://www.adobe.com/cn/products/photoshop/brushes.html)
* [ArtStation上的画笔预设](https://www.artstation.com/marketplace?q=photoshop%20brush&sort_by=trending)
* [DevicantArt笔刷预设](https://www.deviantart.com/search?q=photoshop%20brush)
* [Cubebrush上的画笔预设](https://cubebrush.co/marketplace?categories=354,57)

为了支持Photoshop画笔，向绘画工具属性添加了各种新功能：

* **新的大小和流最小值参数**\
  现在，在启用“笔压力”时，可以指定刀具的最小大小和最小流量。 此参数以基于当前定义的最大大小/流量的百分比工作。 使用笔刷预设时，将自动校准这些设置。\
...
* **支持圆度和翻转**\
  已添加名为&#x200B;**画笔制作器Photoshop**&#x200B;的新SubstanceAlpha，以支持圆度（缩放Alpha的Height）和翻转（镜像两个轴上的图像）等参数。 单击来自ABR文件的笔刷预设时，会自动加载此SubstanceAlpha。\
  ![](../../assets/brush-maker-photoshop.png)\
  ![](../../assets/brush-maker-photoshop-settings.png)
* **针对图层Alpha 通道的新灰度系数校正**\
  Photoshop不会在线性灰度系数空间中混合其画笔描边，这意味着使用Photoshop笔刷预设绘画时，混合和不透明度可能看起来是错误的。 可以在图层上启用新设置，以匹配该行为并应用灰度系数校正。 这将影响用于绘画画笔描边的Alpha，以及图层蒙版如何用于与其他图层混合，但图层的混合模式仍将在线性灰度系数空间中运行。\
...
有关更多信息（例如如何导出ABR文件以及如何导入它们），请查看[笔刷预设](../../painting/presets/photoshop-brush-presets/photoshop-brush-presets-abr.md)文档。
...
除了支持笔刷预设之外，还对图形平板电脑的使用进行了大量改进和修复。
...
* **新的画笔预览模式**\
  视口中的画笔预览现在可以更改为不同的可视化模式。 要更改模式，只需单击上下文工具栏中的新下拉按钮。

  ![](../../assets/brush-outline.png)
* **笔压力曲线**\
  现在，可以在上下文工具栏中定义应如何解释笔压力。 这些新设置控制压力累积的速度，从而允许使用不同的绘画样式。

  * **线性**：无变换，即图形输入板笔提供的检索到的压力。 如果已在Tablet驱动程序设置中定义了笔压力曲线，请使用此设置。
  * **缓入**（默认）：减慢压力的开始，从而更容易绘画细描边或淡描边。
  * **淡入淡出**：减慢压力的开始速度并加快其结束速度，从而更容易绘画柔和或强烈的笔触。

  ![](../../assets/pressure-curve.png)
* **压力按钮不再是下拉列表**\
  我们将笔压力控件更改为简单的开/关按钮。 这使启用和禁用压力更加轻松快捷。
...
现在，Substance Painter将自动展开缺少UV坐标的网格。 这样就可以导入任何类型的几何并立即开始绘制。 我们的展开系统将为每个子网格生成一个UV 岛，同时仍遵循材料分配以创建纹理集。 此功能当前处于测试阶段，以后版本中将继续改进。 自动展开将仅应用于&#x200B;**不使用UDIM工作流**&#x200B;的项目。

* **自动展开**\
  默认情况下，Substance Painter现在会自动为缺少坐标的网格生成UV坐标。 这适用于项目创建和网格重新导入。 但是，可以通过进入[主要设置](https://helpx.adobe.com/cn/substance-3d/unlisted/documentation/spdoc/general-71008262.html)并在&#x200B;**导入选项**&#x200B;下禁用&#x200B;**启用自动UV解封**&#x200B;来禁用此行为。

  ![](../../assets/uv-unwrap-setting.png)
* **展开进度栏**\
  导入网格时，现在会出现一个进度条，指示进程的当前状态。 这还包括展开过程。
...
* **使用软范围滑块进行非固定**\
  直到现在，图形中公开的滑块一直表现得像被夹住了一样。 表示可以输入的值不能超过参数定义的缺省最小值和最大值。

  ![](../../assets/slider-soft-range.gif)
* **支持参数中定义的步骤**\
  调整滑块时，现在将考虑具有具有具有已定义步骤的参数的图形。
* **提高了浮点滑块的数字精度**\
  float滑块现在具有输入值，该值可降至6个小数。 但是，这受浮点精度的限制，这意味着在某些情况下可能会对值输入进行四舍五入。
* **带有动态笔触的新随机种子控件**\
...
* **将输出节点指定为所有通道的蒙版的新用户数据**\
  现在，可以在输出节点上添加新的用户数据，以将其用作所有其他通道的Alpha蒙版。 这与现有的&#x200B;**channels\_system** Alpha类似，但是不需要在Substance图形中创建新的专用输出。 有关详细信息，请参阅[userdata文档](../../content/creating-custom-effects/user-data.md)。
...
* **独立视口焦点**\
  已修改2D和3D焦点(F快捷键)，但出现以下行为：

  * **将鼠标悬停在2D 视图上**：按F键只会聚焦2D 视图。
  * **将鼠标悬停在3D视图上**：按F键只会聚焦3D视图。
  * **将鼠标悬停在视口外**：按F键将同时聚焦2D和3D视图。
...
* **使用Ctrl+Alt+左键单击快捷键滚动停放和Windows**\
  添加了一个新快捷键，它允许在不使用鼠标滚轮的情况下滚动窗口和停放。 此快捷键现在可以使用图形输入板的笔进行滚动
...
* **更新了“Meet Mat”示例项目**\
  Mat已使用新的拓扑结构进行了更新，使其更易于位移。 该Id 图经过重新设计，可提供更多蒙版功能，项目中提供了一组新的相机，可提供新的视角。
...
* **新筛选器**\
  ...
  * **MatFx漫画书**\
    此滤镜根据提供的输入（从base color/扩散到弯曲）模拟阴影线和边缘线。
  ...
  * **MatFx油绘画**\
    受[Emrecan Cubukcu](https://www.artstation.com/emrecancubukcu)工作的启发，此滤镜从输入中读取颜色信息，并根据各种参数将其平移为画笔笔触。 您可以使用多个预设轻松尝试各种变化。 我们建议将其与&#x200B;**烘焙光照环境**&#x200B;滤镜组合使用，或手动烘焙/绘画纹理中的阴影以最大限度地提高效果。
...
* **新阿尔法**\
  除了用于创建新笔刷预设的alpha外（见上文），还集成了两个新的重要Alpha：

  * **画笔制作器Photoshop**\
    这个新的图形通过“动态描边”功能复制Photoshop中一些可用的特定画笔参数。 可以通过控制圆度和翻转或输入图像。 一些抖动参数可用于创建更多变化。 单击来自ABR文件的Photoshop画笔预设时，此Substance图表自动插入Alpha部分。

    ![](../../assets/icon-brush-maker-photoshop.png)
  * **画笔制作器绘画辊**\
    这个新的图形可模拟绘画辊（或简单的色带工具），以便不间断地绘画连续图案。 要更轻松地设置，请查看现有预设或参阅图形说明。 我们建议启用[延迟鼠标](../../painting/lazy-mouse.md)来正确绘制滚动画笔，而不会导致破裂。

    ![](../../assets/icon-brush-maker-paint-roller.png)

    ![](../../assets/paint-roller-text-warning2-optim.gif){width="290px"}
* **新的“UV检查器”生成器**\
  为了分析UV的坐标，集成了一种新型的“UV检查器”生成器。 这样可以更容易理解自动展开生成的UV。

  ![](../../assets/icon-uv-checker.png)
* **新模板和导出预设**

  * **关键帧9+**\
    此导出预设使导出的纹理与新的“关键帧9”功能兼容，该功能可简化纹理和材料的加载和分配。 有关详细信息，请参阅[关键帧文档](https://luxion.atlassian.net/wiki/spaces/K9M/pages/1124335675/Material+Importer)。
  * **Spark AR Studio**\
...
* 升级到Iray2019.3
* [日志]指示Ryzen CPU的过时BIOS导致烘焙期间崩溃
* [ABR]将ABR字母提取到工具架

**已修复：**

* [Baker]如果高多边形网格没有UV，则烘焙失败
* [Linux]自定义鼠标快捷键未保存
* [画笔]轮廓消失，出现一些Alpha形状
* [Tablet]移动滑块时检测不佳
...
*（2019年12月17日发布）*\
摘要：**主要版本，改进了手绘用户体验、使用平板电脑、Beta版中的自动展开(0.3.0)以及手绘的多种新内容**
