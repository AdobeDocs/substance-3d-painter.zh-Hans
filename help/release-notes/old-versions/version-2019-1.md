---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/release-notes/old-versions/version-2019-1.html"
breadcrumb-title: ''
...
在此版本中，我们的画笔引擎现在支持我们所说的动态笔触。 这些类型的描边可以随时生成新的Substance版本，从而创建变化和新效果。 现在可以在资源上为每个新画笔描边绘制新的材料或Alpha。

在绘画工具（绘画、橡皮擦、涂抹或仿制）中加载与Dynamic Stroke兼容的资源时，将显示新的参数组：

![](../../assets/dynamic-stroke-ui.png)

动态笔触支持以下属性（如果公开Substance图形中）：
...
只需浏览工具架并查看位于其旁边的新图标，即可找到与此新功能兼容的资源：

![](../../assets/shelf-icon.png)

与该功能兼容的资源还会自动获得一个名为“**dynamicstroke**”的新标签，以便按照工具架中的关键字轻松对其进行筛选。
...
Substance Painter现在支持&#x200B;**位移**&#x200B;和&#x200B;**网格镶嵌**&#x200B;的实时视口和Iray。 这两个参数都可以在着色器参数下方的&#x200B;**着色器设置**&#x200B;窗口中控制。
...
加载“**材料**”示例项目（通过&#x200B;**文件>加载示例**）以快速试用此新功能：

![](../../assets/height-sculp.gif){width="450px"}![](../../assets/cracks-demo.jpg){width="450px"}

>[!NOTE]
>
> 工具架中添加了一个名为“**Height到正常**”的新筛选器，可用于获取最终法线图（在Substance Painter的本机转换不够强的情况下）。

### 比较蒙版效果

![](../../assets/compare-mask.png)

有时，创建和混合材料会有些困难，这就是为什么我们创建了名为“**比较蒙版**”的新效果。 此效果允许快速轻松地比较两个通道并生成蒙版。
...
若要使设置更加简单，您可以右键单击图层并选择快捷键“**添加Height组合的蒙版**”，以在您的图层上快速添加此新蒙版。 此快捷键还会将Height声道混合模式切换到“正常”，而不是默认的“Linear dodge（添加）”。\
...
我们扩展了对称工具的功能，以处理径向对称。 现在，“对称设置”菜单中提供了新的模式来启用它（在上下文工具栏中提供）。
...
我们还添加了一些预览，以便在开始绘画之前可以更轻松地调整设置：
...
添加了两个带有填充图层和填充效果的新投影模式： **平面**&#x200B;和&#x200B;**球面**。 同时，我们还增加了很多新的参数来进一步控制3D投影的行为。
...
* **平面投影剔除/渐隐**\
  有多种设置可用，可使投影连续或有限。 启用剔除设置后，操纵器周围的虚线框指示投影的定界框，中线是投影的起始处。 缩放投影可以控制它到达的距离以及开始淡化的时间。
...
* **已移动并重命名现有设置**\
  由于这些新的投影，我们稍稍改进了一些设置的运作方式。 例如，“**拼贴**”已重命名为“**UV 展开**”。 现在，只能垂直或水平设置拼贴。 “缩放”、“旋转”和“偏移”现在属于名为“**UV变换**”的新参数组，可在不同投影模式中保持一致。

  ![](../../assets/repeat-mode.png)

  ![](../../assets/uv-transform.png)
* **改进了旋转操纵器全轴模式**&#x200B;现在，它不是绘制显式球体，而是隐藏起来，以避免隐藏下方的纹理。 在轴之间单击可选择允许同时旋转所有轴的球体。\
...
* **纹理集的多选内容**\
  ...
  在多重选择模式下，仍然存在“主”纹理集的概念，这也是为什么会以灰色选择其他元素。 如果需要在保留当前选区的同时切换到其他纹理集，可以使用鼠标中键来实现此目的。
* **在纹理集列表中快速显示/隐藏**\
  您现在可以单击并拖动（如图层堆叠中的）来隐藏或显示纹理集。
* **改进了图层堆叠的UI**\
...
* 现在可以通过文件仿色和位深度旁边的导出窗口中的专用设置禁用&#x200B;**导出时的仿色**。 有关如何及何时应用仿色的详细信息[请参阅导出文档](../../export/export-window/export-window.md)。\
  ![](../../assets/dithering.png)
* **更好的直方图**\
  我们重新设计了直方图生成器。 现在，直方图应显示更准确的信息，并在图层堆叠发生更改后正确更新。\
...
* **新画笔和工具预设**\
  此版本引入了新的动态笔触功能，并添加了一些画笔和工具预设以供使用。
...
* **13个新筛选器**\
  ...
  * **PBR 验证** ：我们添加此筛选器以检查您的PBR材料是否在正确的范围内。 有关详细信息，请查看[PBR指南](https://www.allegorithmic.com/pbr-guide) ！
  * **MatFX剥落绘画** ：模拟旧的绘画开始剥落。 此滤镜输出Alpha，以便与下面的材料轻松混合。
  * **MatFx水滴** ：模拟对象表面上的水滴。 就像下雨后车上的水。
* **7个新生成器**\
  在此版本中，我们添加了一些新的生成器：

  * **Ambient occlusion** ：提供对Ambient occlusion网格图的控制的蒙版生成器。 基于蒙版编辑器。
  * **世界空间法线** ：提供对世界空间法线网格图控制的蒙版生成器。 基于蒙版编辑器。
  * **位置** ：提供对位置网格图的控制的蒙版生成器。 基于蒙版编辑器。
  * **弯曲** ：提供对弯曲网格图的控制的蒙版生成器。 基于蒙版编辑器。
  * **自动拼接器** ：在UV边框、网格弯曲或自定义蒙版输入周围创建拼接的蒙版生成器。
  * **UV纹理密度** ：基于网格多边形的纹理密度输出彩色渐变的助手。
