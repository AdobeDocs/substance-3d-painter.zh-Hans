---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/release-notes/old-versions/version-7-4.html"
breadcrumb-title: ''
...
**Substance 3D Painter 7.4**&#x200B;引入了新的色彩管理工作流程，从而增加了对OpenColorIO的支持。
...
此版本引入了色彩管理，并支持[OpenColorIO](https://opencolorio.org/)（简称OCIO）版本2。

这一新的工作流程允许管理和校准从导入到导出以及视口内部的颜色，从而更轻松地在不同应用程序之间匹配任何内容。
...
* **导出设置**\
  导出纹理时，色彩管理的通道将在文件名中显示借助新关键字&#x200B;**$colorSpace**&#x200B;使用的色彩空间的名称。
...
现在，可以取消停靠2D和3D视图，以将其移动到其他位置。 例如，当2D 视图位于另一个屏幕上时，在主屏幕上显示3D视图。
...
* **与色彩管理兼容**\
  非停放视图具有自己的色彩管理显示变换，因此可以更轻松地在不同显示器中进行管理。
...
**SpaceMouse®**&#x200B;是一款通过3D连接的设备，允许以更直观、友好的方式操作3D视口相机。 现在，它支持通过Painter直接即插即用。
...
自动展开已更新，新选项可用于改善对具有扩展曲面的3D模型的支持。

名为&#x200B;**避免拉长的UV 岛**&#x200B;的新设置通过拆分可能太长的UV来更好地利用UV 岛空间。
