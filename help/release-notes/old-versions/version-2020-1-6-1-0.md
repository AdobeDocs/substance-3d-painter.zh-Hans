...
**Substance Painter2020.1 (6.1.0)**&#x200B;提供了全新的导出器、Python脚本、新的弯曲Baker、新内容和许多其他工作流程改进。
...
此选项卡控制每个纹理集的常规设置和特定设置。

* **新的全局设置**\
  全局设置是一组在所有纹理集之间共享的参数。 它们可用于快速分配或更新参数，而无需分别手动编辑每个纹理集。 全局设置包含以下参数：

  * **输出目录**：定义将在其中生成纹理的目标位置。
  * **输出模板**：定义用于配置导出纹理的命名和打包的预设。
  * **文件类型**：定义导出纹理的文件格式和位深度。
  * **大小**：定义纹理集将导出时采用的最大纹理分辨率。
  * **填充**：定义如何生成UV 岛以外的信息。
...
* **每个纹理集的设置和参数覆盖**\
  ...
  单击笔图标允许覆盖特定参数以更改其值。 再次单击它将禁用覆盖并将值重置为默认值（继承自全局设置）。

  ![](../../assets/param-override-demo.gif)
* **选择要导出的纹理**\
  纹理集导出参数下方是将生成的纹理列表。 使用此列表，可以通过选中或取消选中来精确选择应生成哪个文件。 该列表还显示继承自导出配置或输出模板的每个纹理的文件格式和位深度。 使用笔图标可覆盖特定纹理的文件格式和位深度。
* **保存设置而不导出**\
...
* **导出的纹理列表**\
  该选项卡列出了每个纹理集要导出的每个纹理，并会考虑被覆盖的参数和忽略/排除的纹理。 这是在启动导出流程之前确保一切正常的好方法。 当导出过程开始时，界面将自动切换到此选项卡。
...
从[资源](../../interface/assets/assets.md)中拖放材料时，有一种新的&#x200B;**贴花**&#x200B;模式可用。 它在&#x200B;**投影**&#x200B;模式下创建新填充图层，从而更轻松地定位图案和图像。 要使用它，只需在按住&#x200B;**ALT键盘快捷键**&#x200B;的同时从搁板中&#x200B;**拖放**&#x200B;任何材质来预览操纵器，并在贴花位于所需位置时释放鼠标。 释放鼠标按钮后，将创建一个新填充图层。

现在，我们还将从Substance Source中选择的默认工具架包含5个贴花。 如果您想了解有关此新贴花工作流程的更多信息，请查看[此文章](https://magazine.substance3d.com/effortless-grunge-look-with-new-substance-source-parametric-decals/)。
...
在此版本中，对Baker进行了改进，Ambient occlusionBaker得到了一些新的参数。

* <b>从Baker新建弯曲</b>\
  旧的Baker现已弃用，并且已被最近添加到Substance Designer的新“来自网格”版本取代。 要了解有关新Baker设置的更多信息，请查看文档页面。\
  新Baker具有以下功能：

  * <b>更准确</b>：计算的弯曲更准确，并生成逼真的值。
  * <b>网格联系人</b>：网格之间的渗透现在会生成弯曲信息（与以前的Baker不同）。
  * <b>高多边形网格</b>：现在直接从高多边形网格烘焙细节，而不是从法线图转换。
  * <b>性能</b>：新Baker使用射线追踪计算细节，这意味着它受益于GPU 射线追踪加速(RTX/Optix)。

  ![](../../assets/baker-comparison-curvature.jpg){width="500px"}

  出于兼容性原因，仍然可以通过Baker设置访问旧的弯曲Baker。 选择&#x200B;**从法线图生成**&#x200B;设置以使用旧Baker。

  ![](../../assets/curvature-baker-setting.png)
* <b>已改进Ambient occlusion</b>\
  已改进Baker以支持新设置：

  * <b>地面平面</b>：模拟网格下方的平面以创建来自地面的阴影。 有关详细信息，请参阅Ambient occlusion文档。
  * <b>忽略背面</b>：Ambient occlusion时，有一个新的网格名称后缀可忽略特定对象。 例如，这允许忽略浮动几何细节。 有关更多信息，请参阅“按名称匹配”文档
...
* **导出原始网格拓扑或导出带有曲面细分的网格**\
  导出网格时，现在可以选择应用为位移效果生成的曲面细分。 可能的设置包括：

  * **没有位移/曲面细分**：导出没有网格子划分的基本网格。 如果禁用&#x200B;**应用三角化**，则将导出原始网格三角形、四边形或n-gon。
  * **具有位移/曲面细分**：导出具有子分区的网格。 如果启用了&#x200B;**重新计算顶点法线**，则将调整网格法线以匹配位移偏移。

  ![](../../assets/export-mesh-settings.png)
* **层次结构和网格名称场景**\
  现在将保留原始场景层次结构和导入网格中的对象名称，并将其重新应用于导出的网格。
* **以FBX文件格式导出**\
  现在，可以将项目网格与其他文件格式一起导出到FBX中。\
...
现在，借助新设置，可以更轻松地控制自动展开：

* **在项目创建期间使用自动UV解封**\
  创建项目时，现在有一个新设置，可用于启用或禁用新的自动展开流程。 将3D 网格重新导入现有项目时，此设置也可用。

  ![](../../assets/uv-new-project-setitng.png)
* <b>高级展开设置</b>\
  在用于启用自动展开的复选框旁边，有一个新的<b>选项</b>按钮。 它会打开一个新窗口，其中包含用于控制展开过程的高级设置，从而允许将现有网格信息保留在过程的不同部分，而不是总是像以前那样重新计算所有内容。 有关更多信息，请参阅专用文档。 当前设置包括：

  * <b>接缝</b>：定义是保留还是重新生成现有接缝。
  * <b>UV 岛</b>：定义是保留还是重新生成现有UV展开。
...
* **5种新贴花素材**\
  我们添加了5个直接来自Substance Source的新材料，以试用新的贴花功能：
...
* [Livelink UE4]仅最后一个材料用于组合网格中的所有纹理集

**已知问题：**

* [导出]&#x200B;[Dimension]&#x200B;[Skecthfab]不应导出禁用的纹理集
* [崩溃]在着色器图层上绘制蒙版后更改材料
