...
<b>Substance 3D Painter 11.1 </b>带来了新的功能区路径工具，该工具具有专用内容、填充图层和效果对称、位移的物理大小以及对Vulkan图形API的支持。

发行日期：<b>2025年11月18日</b>

>[!NOTE]
>
> 此版本的Painter将图形API从OpenGL切换为Vulkan。 此更改可能会影响应用程序支持的GPU，特别是基于GPU的射线追踪的烘焙。
> 
...
这一新工具为诸如沿路径放置文本、沿路径放置完美的渐变以及轻松创建自己的高级修剪以环绕网格等新行为打开了大门。\
...
* <b>功能区是跨所有表面工作的连续路径</b>\
  它功能区是一种工具，允许沿路径重复或纹理。 它可以跨任何类型的曲面和几何形状工作，即使网格零件未连接也是如此。

  ![](../assets/ribbon_robot_1.jpg)
* <b>制作重复图案和渐变</b>\
  此新工具能以各种方式重复图像，无需接缝或剪切，适用于渐变和干净的图案。
...
* <b>可用的角类型</b>\
  打破正切以创建转角时，根据需要，可使用多种形状 — 从经典破断到平滑车削。
...
* <b>与动态笔触工作流程兼容</b>\
  ...
  系统还提供了两个名为<b>自定义功能区灰度</b>和<b>自定义功能区材料</b>的新工具预设，以便轻松访问这项功能。

  ![](../assets/ribbon_custom_preset.gif)
* <b>与对称兼容</b>\
  与其他类型的工具一样，该功能区路径也与对称功能兼容。
...
此版本包括75个利用新“功能区”功能的新工具预设。 为使预设更易于发现，已在<b>属性</b>窗口中添加了新的预设类别。
...
* <b>新内容</b>\
  此版本中添加了专门用于“功能区”工具的75个新工具预设，作为默认内容的一部分。 这些预设可直接在<b>资源</b>窗口画笔部分下使用，或通过<b>属性</b>窗口中的新类别快捷键使用。\
  这些预设包括：
...
填充图层和效果现在支持使用3D投影模式对称。 可通过上下文工具栏中的“对称”菜单或<b>属性</b>窗口中新添加的“对称”部分启用该功能。

* <b>在填充图层</b>上对称\
  在填充效果和图层中使用基于3D的投影模式时，现在可以启用对称。 镜像和径向对称均可用。

  ![](../assets/symmetry_fill.jpg)
* <b>通过上下文工具栏或“对称”窗口启用属性</b>\
  对称可以通过<b>上下文工具栏</b>菜单（类似于绘画工具）或通过<b>属性</b>窗口激活新的专用部分。

  ![](../assets/symmetry_contextual.png)

  ![](../assets/symmetry_properties.png)
* <b>翻转文本和徽标的输入资源</b>\
  填充图层和效果对称还受益于允许您翻转输入图像或X/Y轴的新选项。 例如，这允许镜像文本，但仍使其两侧均可读。

  ![](../assets/mirror_symmetry_flip.gif)
* <b>改进了对称设置界面</b>\
  重新设计了对称设置的界面，以方便阅读和使用。 例如，每个轴滑块都有自己的线条，这有助于提高精确度。 此外，还缩小了径向显示尺寸，以占用更少的空间。
...
* <b>现在，在Windows和Linux上使用Vulkan图形API代替OpenGL</b>\
  Painter现在使用Vulkan graphics API在视口和计算纹理中进行渲染。 此开关应能改善应用程序的总体性能。 这还将使今后更容易整合新功能。
* <b>通过Vulkan进行烘焙的GPU 射线追踪</b>\
  射线追踪(DRX)和Optix已替换为射线追踪（通过Baker中的Vulkan图形API）。 此更改意味着基于GPU的射线追踪现在可在AMD GPU和Linux操作系统上使用。\
...
* <b>改进了单个大三角形的性能</b>\
  直到现在，Painter还会在非常低的多边形网格或具有非常大和/或长三角形的网格上苦苦挣扎。 现在情况已经不同了。 使用单个四网格（例如，创建拼贴纹理）应该已经不再是个问题。
* <b>改进了默认画笔形状</b>\
