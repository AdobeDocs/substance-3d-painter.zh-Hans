...
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/release-notes/version-12-0.html"
...
<b>Substance 3D Painter 12.0</b>在图层堆叠中直接提供纹理拼合、全新的变形投影自动模式、一组改版的后处理效果以及经过改进的项目创建和设置工作流程。
...
现在，图层堆叠的右键单击上下文菜单中提供了新的<b>拼合</b>操作。 通过将多个图层编组(<b>Ctrl/Cmd + G</b>)并创建拼合副本(<b>Ctrl/Cmd + M</b>)，可快速合并这些图层。 源组将自动禁用，选择是将其删除，还是将其另存为<b>智能素材</b>以供日后编辑。

图层栈栈的拼合元素也可以直接导出到磁盘，以便在其他应用程序中快速迭代。 可以通过图层堆叠的右键单击菜单单独或批量导出组、图层或蒙版。
...
* <b>将纹理拼合并导出到磁盘</b>\
  右键单击菜单中的专用导出操作可烘焙图层、蒙版或组的拼合结果，并将其直接保存到磁盘。 如果要将烘焙内容传输到其他应用程序，而无需通过完整的纹理导出管道，此功能非常有用。
* <b>批处理操作</b>\
  可以一次选择多个图层、组或蒙版，并在单个操作中单独拼合或导出，从而高效地一步处理图层堆叠的大部分内容。
...
贴花现在可以自动适应复杂表面，从而减少手动调整的需要。 当变形投影处于活动状态时，可在上下文工具栏中使用<b>几何变形</b>切换开关。

* <b>上下文工具栏中的新参数</b>\
  每当变形投影模式处于活动状态时，上下文工具栏中都会显示新的<b>几何变形</b>切换开关。 可以随时将其关闭，而无需重置当前投影设置。

  ![](../assets/v12_warp_toolbar.png)
* <b>自动包装到网格表面</b>\
  启用后，变形投影将自动遵循基础网格的曲率和拓扑。 将投影拖过曲面可使其平滑地符合几何形状，从而显着减少将贴花放置在复杂或弯曲形状上时所需的手动微调量。

  ![](../assets/v12_warp_to_geometry.gif)
* <b>保留本地变形</b>\
  在编辑变形网格顶点时，变形到几何模式将尝试保留预定义变形，以确保始终投影相同的形状。
...
现在，可通过<b>显示设置</b>窗口中提供的一组全新的后期处理效果来增强Painter中的渲染。 现在可以使用诸如<b>镜头眩光</b>和<b>胶片颗粒</b>的新添加项，以及改进的<b>字段深度</b>和<b>眩光</b>效果等。
...
* <b>新的后期处理效果</b>\
  可以在<b>显示设置</b>窗口中单独启用和配置所有后期处理效果。 效果按堆叠顺序应用，每个效果可以单独打开或关闭，便于组合和实验。
...
* <b>新效果列表：</b>
  ...
  * <b>眩光</b>：在光源周围创建光线。
  * <b>镜头眩光</b>：模拟明亮光线在相机中照射时镜头的光学反射。
  * <b>横向色差</b>：模拟镜头瑕疵在图像边缘产生的彩色边纹。
  * <b>晕影</b>：调暗帧的角部和边缘，以便将注意力吸引到中心。
  * <b>锐化</b>：增加边缘对比度，使渲染的图像看起来更清晰。
  * <b>胶片颗粒</b>：叠加细微的杂色以复制模拟胶片的纹理。
  * <b>色调映射</b>：将明亮度值重新映射到可显示的范围，以获得更具电影效果的外观。
...
“新建项目”窗口和“项目设置”对话框经过重新设计，更加易于导航。 为提高可读性，对参数重新排序和分组，改进了网格重新导入工作流程，以减少迭代项目时的重复步骤。

* <b>改进的新项目窗口</b>\
  新项目窗口中的参数经过重新组织和排序，以便最常用的设置更加突出。 现在，可以更轻松地扫描整体布局，从而减少配置新项目所需的时间。
* <b>在项目设置中重新导入网格的新工作流程</b>\
  项目设置中的新复选框<b>重新导入网格</b>允许更轻松地重新导入项目网格，这是因为当前正在自动保存和预填充以前加载文件的文件路径。
