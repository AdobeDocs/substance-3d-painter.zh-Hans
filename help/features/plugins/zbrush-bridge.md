...
使用SubstanceBridge增效工具，您无需经历以下冗长的过程：导出单独的低多边形和高多边形文件，将它们导入Painter，以及配置和运行烘焙。

要开始使用“画笔到Painter Bridge”，请执行以下操作：

1. 确保至少安装了版本2026.2.0的ZBrush。
1. 通过确保选中&#x200B;**Python > zbrush_painter_plugin**，启用Painter中的插件。
1. 从ZBrush中，**发送到Painter**&#x200B;可在&#x200B;**纹理>SubstanceBridge**&#x200B;中找到
...
| 活动 | 仅发送当前选定的子工具 |
| 发送PolyPaint | 将PolyPaint转换为纹理映射并将其应用为Substance中的填充图层，您可以在其绘画并与之混合。 |
| 平滑法线 | 在导出时平滑正切法线，使多面网格在Substance中显得平滑，与游戏引擎呈现它们的方式匹配。 关闭以查看几何的实际多面。 |
| 自动烘焙映射 | 在模型到达后自动运行Substance的烘焙算法，从高/低法线图比较中生成ambient occlusion、弯曲和其他详细图。 |
| 强制自动展开 | 在到达的每个子工具上触发Substance的UV展开算法。 如果您的模型已经有良好的UV，请将其关闭，因为这样会覆盖它们。 |
| 被除数 | 控制发送细分级别。 “当前”仅发送所显示的级别。 “低”和“高”选项用于发送最低和最高的烘焙级别，是大多数工作流程的建议选项。 |
| 纹理集 | 控制UV空间在Substance中的划分方式：每个子工具（每个子工具一个纹理集）或每个多边形组（每个子工具内每个多边形组一个纹理集）。 |

当Painter收到模型时，如果启用自动烘焙，则将启动烘焙。 模型的最低细分是以低多边形网格作为输入网格，最高细分以高多边形作为烘焙细节。 ZBrush可以处理的多边形数量比Painter多得多，因此请确保低模网格具有最佳工作大小（具体取决于机器，但最好小于100万个）。

Painter中的纹理集表示材料分配。 一个纹理集等于一个UV空间。

* 每个子工具为每个子工具创建一个纹理集（所有子工具部分将共享相同的UV空间），这是更简单的选项。
* 每个PolyGroup为每个子纹理集内的每个PolyGroup创建一个Tool，从而更精细地控制材料分配。
