...
3Dconnection的SpaceMouse®是一种允许在3D环境中轻松导航的设备。 它可用于在应用视口中对相机/3D模型进行操作。
...
>[!NOTE]
>
> 使用紧凑模型且需要频繁旋转环境图的用户可以选择将SHIFT键分配给左键。

## 教程

## 概述

主控制旋钮或SpaceMouse®允许您以常规鼠标/光笔和键盘控制无法实现的方式旋转、平移和缩放视口。 该装置可与鼠标和带光笔的平板电脑结合使用。
...
| 相机模式 | 在3D空间中自由控制相机。 |
| 目标相机模式 | 控制摄像机始终以3D空间中的点为目标。 |
| 直升机模式 | 在3D空间中控制直升机。 |
| 锁定水平线 | 锁定相机，使水平线始终保持水平。 Painter已在其设置中提供类似的选项，但可在此处单独控制它。 默认情况下，此选项处于锁定状态。 |
...
| 设置 | 描述 |
| --- | --- |
| 自动 | 自动移动枢轴或相机目标，如果关闭，它始终紧贴网格源枢轴。 |
| 始终显示 | 即使不与设备交互，枢轴点也始终显示在3D视口中。 |
| 动态显示 | 仅在与该设备交互时，该枢轴点才会显示在3D视口中。 这是默认选项。 |
| 隐藏 | 在3D视口中完全删除透视点。 |
