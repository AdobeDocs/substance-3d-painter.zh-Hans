...
本页列出了与OpenColorIO (OCIO)相关的色彩管理设置。
...
| **substance\_3d\_painter\_bitmap\_export\_floating** | 用于指定导出纹理时使用的色彩空间的角色。 |
