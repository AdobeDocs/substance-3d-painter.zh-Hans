...
应用程序中的色彩管理是通过[OpenColorIO](https://opencolorio.org/)（简称OCIO）版本2的集成来处理。 OCIO是电影和动画中转换和显示颜色的标准。 要启用色彩管理，只需创建一个新项目或打开一个现有项目并启用专用设置即可。
...
| --- | --- |
| **色彩空间** | 在其中定义颜色的坐标系。 |
| **工作空间** | 在应用程序内用于混合纹理、绘画等的色彩空间。 |
| **显示变换** | 显示变换将线性颜色从工作空间转换为显示器的色彩空间，以感性方式（肉眼可见）显示颜色。 显示变换通常包括一个色调映射通道，用于压缩颜色以适合屏幕允许的有限值范围。 |
| **配置** | OCIO配置文件。 它定义了什么是工作空间、色彩空间列表和显示变换列表。 |
| **ACE** | ACE代表Academy 颜色编码系统，是许多应用程序中交换数字图像文件的标准。 默认情况下，此标准的两个版本包含在应用程序中。 |
...
2D视图和3D视图均进行色彩管理，并在视口顶部提供专用设置来控制要变换使用的显示：

![](../../assets/viewport-cm.png)

* **左键**：启用/禁用视口的显示变换。 如果禁用，则视口将以Raw/Passthrough格式显示颜色。 默认情况下，此按钮处于启用状态。
* **右侧下拉列表**：指定要用于转换颜色的显示变换以将其显示在屏幕上。 默认值基于OCIO配置。 此设置未随项目一起保存，因为它可能与监视器相关。
...
在[显示设置](../../interface/display-settings/display-settings.md)中，启用&#x200B;**覆盖环境图色彩空间**，然后在列表中选择与您的资源匹配的色彩空间。
