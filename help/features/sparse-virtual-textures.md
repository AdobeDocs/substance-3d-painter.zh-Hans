...
从版本&#x200B;**2018.3**&#x200B;开始，Substance 3D Painter在其实时视口中使用&#x200B;**Sparse Virtual Texture** ( **SVT** )来管理大量纹理。 此技术允许仅从特定角度需要的纹理输入和输出，以便在GPU内存上保持特定的占用空间。 它可以提高具有大量纹理集（或UDIM）的项目的性能。

## 支持的平台

![](../assets/sparse-settings.png)

稀疏纹理依赖于特定的硬件配置才能完全实现性能。 如果当前配置不支持该功能，Substance 3D Painter将&#x200B;**回退**&#x200B;到软件实现（这将降低精确度和性能）。

在[设置](../interface/settings/settings.md)中，可以强制Substance 3D Painter使用软件回退而不是硬件加速。

以下是支持硬件加速Sparse Virtual Texture的配置：
...
## 为什么Substance 3D Painter使用Sparse Virtual Texture？

Substance 3D Painter使用其主要引擎来计算纹理，然后在视口中显示。 这意味着引擎和视口必须共享GPU内存(VRam)才能计算和显示这些纹理。 项目包含的&#x200B;**纹理集**（或UV 平铺）越多，视口所需的内存就越多。 如果视口在GPU上占用了太多内存，则主引擎没有足够的空间来计算纹理，因此必须将纹理逐出到系统内存(Ram)中。 这将导致表现不佳和计算缓慢。

SVT的目标是为视口在GPU内存上的使用量制定预算，从而让主要引擎有尽可能多的空间进行计算。 该系统的优势在于，它还解锁了将更大的项目加载到Substance 3D Painter中，同时仍可正常工作的功能。

## 稀疏纹理的工作原理是什么？

Sparse Virtual Texture是一种不完整的纹理。 这意味着应用程序仅将部分纹理加载到内存中。 只加载需要的内容，其余内容放入系统内存或磁盘（缓存）。 再次需要时，将从缓存中检索纹理并将其放回视口。 要使传输速度足够快，系统必须依赖&#x200B;**mipmaps**&#x200B;才能在不同纹理分辨率之间快速跳转。 这就是为什么快速进入视口可能会首先显示模糊的纹理，然后在几秒钟后提高品质。

有关更多技术知识，请参阅： [Sparse Virtual Texture](https://silverspaceship.com/src/svt/) 。
...
>[!WARNING]
>
> 如果旧项目的着色器不是最新的，则旧项目可能会显示白色闪光灯。 请参阅此页面了解更多信息： [移动相机时闪光网格](../technical-support/technical-issues/rendering-issues/mesh-flash-to-white-when-moving-camera.md)。
