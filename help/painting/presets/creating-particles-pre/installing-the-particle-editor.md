---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/painting/presets/creating-particles-presets/installing-the-particle-editor.html"
breadcrumb-title: ''
description: 了解如何安装适用于Substance 3D Painter的粒子编辑器增效工具以创建自定义粒子画笔预设。
helpx_creative_field: ""
helpx_description: Painter > Painting > Presets > Creating particles presets > Installing the particle editor
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: 安装粒子编辑器
user-guide-description: ''
user-guide-title: ''
source-git-commit: 9f20406f682e0e6a2e9a423e81c5ecfc7430ecfd
workflow-type: tm+mt
source-wordcount: '98'
ht-degree: 0%

---


# 安装粒子编辑器

要创建和编辑粒子系统行为，需要使用PopcornFX编辑器。\
从以下链接之一下载编辑器： [镜像1](http://downloads.popcornfx.com/updates/1.5.5.20787/PopcornFx-Editor_Setup_v1.5.5.20787.exe)，[镜像2](https://www.dropbox.com/s/z1vwzvk2ywlug4p/PopcornFx-Editor_Setup_v1.5.5.20787.exe?dl=1)。

>[!NOTE]
>
> Painter使用旧版本的编辑器，因为它需要特定版本的支持。 有关详细信息，请参阅[官方维客](https://wiki.popcornfx.com/)。
