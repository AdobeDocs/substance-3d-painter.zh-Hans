...
将网格复制并粘贴到包的“网格”文件夹中。 然后在编辑器中打开网格并单击“生成”。
...
物理演进器可处理场景中背景网格的冲突。 在Substance 3D Painter中，场景将是您的网格。

首先在物理演化器中将“WorldInteractionMode”设置为“OneWay”，以启用粒子冲突。 然后创建一个名为“OnCollide”的事件，物理演化器将在与场景发生碰撞时触发它。

在Substance 3D Painter中，场景是您正在处理的模型，所有名为“OnCollide”的事件将被当前画笔的发射器粒子系统覆盖。

#### 如何从相机激发粒子

在视口顶部，启用第4个按钮“在相机平面上约束衍生作品”。

默认情况下，Substance 3D Painter将从相机中触发发射器。

#### 如何像雨一样在顶部发出粒子

如果启用，则禁用“在相机平面上约束衍生项目”。

现在，Substance 3D Painter将创建一个名为“Global”的粒子属性，并在源位置衍生您的粒子。

要在网格顶部繁殖，请添加一个形状Sampler BOX或CYLINDER，将其放在顶部，并在Spawner Script中对其取样。
...
在编辑收件人时，要更接近Substance 3D Painter工作流程，您可以设置编辑器以覆盖衍生的粒子系统。

在接收器的树视图中，选择“编辑器属性”，然后启用“UserOverSpawn”，并在“OverSpawnEffect”中选择发射器。

您仍然必须打开发射器以设置“OnCollide”事件，从而生成您当前正在编辑的接收器。
...
在Evolver脚本中，使用网格Evolver提供的参数坐标对Sampler形状“投影”进行取样：

UV= 网格.sampleTexcoord(pCoords)；
...
Normal = normalize(网格.sampleNormal(pCoords))；
...
不被Substance 3D Painter使用，但对网格进行粒子投影和取样其他字段必不可少。

#### 如何在网格上投影粒子

在接收方的“状态\_0”中添加投影演化器。

每个帧的投影Evolver都会将粒子投影到形状Sampler的最近表面上。

投影演化器可以在“OutputParametricCoordsField”指定的粒子字段中填写投影的参数坐标（请参阅“pCoords”粒子字段）。

它可以将一个矢量以“ReprojectedField”的形式重投影到网格的表面。

在这里，我们要将粒子投影到Sampler形状“网格”上，在int3粒子字段“pCoords”中填写参数坐标，并将“Velocity”投影到表面上：
...
要对脚本中的内容进行取样，只需在脚本中编写“网格.sample~内容~(pCoords)”，此处为文档：
...
在Substance 3D Painter中，选择“文件”>“导入粒子”或Ctrl-Alt-R，然后在Pack中选择发射器.pkfx或Receiver.pkfx。

Substance 3D Painter将自动检测要求（粒子字段、OnCollide事件），以确定您的pkfx是发射器、接收器还是不兼容。

现在，您应该可以在工具架中看到您的发射器/接收者。

#### 如何使用可行的粒子大小调试粒子

由于“大小”粒子字段必须介于0和1之间才能成为Substance 3D Painter中画笔大小的倍数，因此粒子在编辑器中会变得太大。 因此，请在Spawner脚本中添加一个设置为0.01的自定义字段浮点“BBSize”，将其用作“SizeField”，以便在公告板粒子渲染器中更好地查看粒子。

#### 如何不搞乱进化秩序

演化器的顺序可能非常重要。

例如，您可能希望始终将最后两个演化器设为投影演化器，然后是脚本演化器，后者使用由UV演化器生成的pCoord对投影和普通代码进行采样。

请记住，演化的顺序实际上就是帧内的执行顺序，Substance 3D Painter将收集粒子字段值和每个帧的结尾。

#### 如何对法线图取样

Substance 3D Painter会将所有名为“NormalMap”的纹理取样器替换为网格法线图（如果已导入）。

Substance 3D Painter将不能访问所有其他纹理，这是您目前可以拥有的唯一纹理。

添加名为“NormalMap”的纹理后，您可以在脚本中对其进行取样：
...
由于更新之间的时间很长，粒子的移动会更大，因此画笔在Substance 3D Painter中的运行速度会更快。
...
如果增量时间较大，则帧之间的粒子移动也较大。 因此，在Substance 3D Painter，可能会出现小污点而不是直线。

发生这种情况的原因是，Substance 3D Painter将在每个帧的末尾为每个粒子绘制一个描边点，而不会在最后一个帧和当前粒子之间为每个绘制线条。
...
Substance 3D Painter将收集粒子系统的粒子属性，并在物理画笔参数中公开这些属性：

<http://www.popcornfx.com/wiki/index.php/Particle_effect_attributes>

在PopcornFX中，有一个名为“Attributes in Evolude”（属性在演进中）的功能，允许您访问“Evolude”脚本中的属性：不要这样做。 而应在Spawner脚本中创建粒子字段并将属性存储在它们中，然后在Evovler脚本中使用这些粒子字段。 （此问题可以在将来修复）

#### 如何检测有问题的粒子

您绝不应该有带有奇怪粒子字段值的粒子，因此请确保您在不同时间中断有问题：
...
要从第一个帧获取有效的pCoordsUV和Normal，请将以下内容添加到您的Spawner脚本：
