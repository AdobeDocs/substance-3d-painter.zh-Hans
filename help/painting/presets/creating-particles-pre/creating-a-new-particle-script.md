---
helpx_url: "https://helpx.adobe.com/cn/substance-3d-painter/painting/presets/creating-particles-presets/creating-a-new-particle-script.html"
breadcrumb-title: ''
description: 了解如何在Substance 3D Painter中创建新粒子脚本以定义自定义粒子画笔行为和效果。
...
* 粒子系统“\_Template”，它是发射器的基础发射器。
* 粒子系统“\_Receiver”，它是接收器的基本模板。
* 用作场景默认背景的球体网格

“\_发射器”和“\_接收者”已“为Painter做好准备”。 它们已经配置了必要的演变、字段、背景等……

## 导入您的网格

PopcornFX仅支持&#x200B;**FBX** ，请确保以此格式导出您的网格。 在导出步骤中，检查网格的大小以尝试与“现实世界”中的正确单位匹配。

将其复制粘贴到项目的“网格”文件夹中（在PopcornFX中，您可以右键单击“网格”文件夹并选择“打开文件位置”）。

返回编辑器，打开网格（双击它），然后单击“**生成**”。 关闭窗口，然后保存更改。

## 发射器/接收者编辑

我们将复制现有的粒子系统，并对它们进行调整以正确地考虑新的网格。

右键单击粒子系统“\_发射器”（在“粒子”文件夹中），然后选择“仿制”（或“复制”）以创建您自己的发射器。

打开它，然后在“粒子树视图”窗口（左下角）中，选择“ **Layer\_Model**”，它应位于： “编辑器属性=>背景=> 3D图层”。

然后，在“节点属性”窗口中，将“dummymesh.fbx”替换为您的模型。 保存修改(“文件”(File)=>“保存”(Save))并关闭发射器窗口。

现在，**克隆“\_Receiver** **”**（在“Particles”文件夹中）以从此文件夹创建您自己的接收器。

打开它，对于发射器，在“Layer\_Model”中用您的模型替换虚拟网格。 我们&#x200B;**修改了屏幕中显示的网格** **&#x200B;**&#x200B;，但是我们还需要修改&#x200B;**粒子使用的网格** **&#x200B;**&#x200B;。
...
打开发射器（之前复制的发射器），然后在“粒子树视图”窗口中，单击“事件”，其位置应为：“粒子效果=>生成器”。 然后点击“Extern”（外部），用您的接收器替换接收器。\
完成了！ 现在，如果您选择（发射器或接收器的）3D视图，可以通过按“间距”按钮创建粒子。

## 可选：修改接收者行为

打开接收者，然后在“粒子树视图”窗口中，选择“ CParticleEvolver\_Script ”（最上面专门针对您的脚本：），它应位于： “粒子效果=>图层\_1 =>状态\_0”。

在“Specialized Node Editor”窗口中，在函数中添加“Life = 0.5；”以更改粒子的生命周期。 然后使用“Ctrl+s”快捷键保存脚本。 您应该能够注意到3D视图中的差异。
...
现在，您应该可以在工具架中（在“发射器”和“接收者”选项卡中）看到您的发射器/接收者。
