...
![显示投影在参数列表旁的网格上的svg文件的图像](../assets/svg_overview.png)

可以像导入Painter中的常规图像一样导入矢量图形文件（<b>.svg</b>和Illustrator <b>.ai</b>）。 我们提供一些设置，用于调整图形的外观，使其更好地契合其他纹理效果。
...
在[图层堆叠](../interface/layer-stack/layer-stack.md)内使用SVG和AI文件时自动转换为像素图像（取决于所选设置）。 这是一个非破坏性的过程，更改分辨率或更新源文件将相应地更新最终结果。
...
|  | 方形纵横比 | 如果裁剪区域由<b>资源边界</b>定义，则此设置可确保保留原始比例，从而避免在将SVG渲染为方形图像时出现任何不正确的拉伸。   此设置可能会使某些元素意外可见。 要避免此问题，请禁用此设置，而改为在填充图层/效果中手动调整UV设置。 |
...
编辑矢量图形的范围时（请参阅上面的设置），将显示一个窗口，其中包含要选择的元素列表，以指定在最终渲染的图像中包括或排除的内容。
